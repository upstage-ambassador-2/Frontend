...
- persona 카드는 이름, 관계, 이메일 연결 상태, 톤/키워드, 톤과 메모 또는 선호 구조 요약, 마지막 작성 시점, 발송 이메일을 표시한다.
- 사람 추가 또는 수정 시 이름, 이메일, 관계, 역할, 톤, 키워드, 금지 표현, 선호 표현, 메모를 입력한다.
- 사람 추가 또는 수정 저장 시 이름이 비어 있거나 이메일 형식이 올바르지 않으면 저장을 막고 폼 안의 inline 오류와 toast를 함께 표시한다.

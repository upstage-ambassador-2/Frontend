...
Sidebar, Topbar, 내부 라우팅, 검색 진입점, 사용자 정보, 레이아웃 기본 동작을 검증한다.
...
| SHELL-02 | P0 | UI | [ ] 각 메뉴 클릭 시 대응 화면으로 전환되고 활성 항목 스타일이 갱신된다 | active class 또는 aria-current 검증 |
...
| SHELL-07 | P1 | UI | [ ] `자주 보내는 사람` 목록의 persona 클릭 시 Compose로 이동하고 해당 persona가 선택된다 | recipient card, tone/length preset 동기화 |
...
- 라우트 상태만으로 동작하더라도 E2E에서는 사용자가 관찰 가능한 화면 전환을 기준으로 검증한다.
